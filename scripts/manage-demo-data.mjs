...
function buildItems(seededGroups) {
  ...
  for (const group of seededGroups) {
    ...
    athletes.forEach((athlete, athleteIndex) => group.schedule[athleteIndex].forEach((planIndex, dayOffset) => {
      ...
      items.push({ PK: `ATHLETE#${athlete.id}`, SK: `SESSION#${coach.id}#${date}`, entityType: "SESSION", athleteId: athlete.id, coachId: coach.id, date, content, updatedAt: now, seedId: SEED_ID });
    }));
  }
  return items;
}
